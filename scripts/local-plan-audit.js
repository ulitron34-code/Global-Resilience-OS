...
const phases = {
  ...
  'phase-3': ['backend/domain/impactGraph.js', 'backend/domain/entityResolution.js', 'backend/domain/evidenceClassification.js', 'backend/domain/calibrationBenchmark.js', 'backend/domain/benchmarkPlan.js', 'backend/domain/backtesting.js', 'backend/domain/sensitivityAnalysis.js', 'backend/domain/uncertainty.js', 'docs/TEMPORAL_IMPACT_GRAPH.md', 'docs/BACKTESTING.md'],
  'phase-4': ['backend/domain/playbooks.js', 'backend/domain/playbookReadiness.js', 'backend/domain/actionPlanStore.js', 'backend/domain/decisionEvidence.js', 'backend/domain/actionLibrary.js', 'docs/DECISION_PACKAGE.md', 'docs/ACTION_LIBRARY.md', 'docs/OUTCOME_FEEDBACK_LOOP.md'],
  ...
  'phase-7': ['backend/domain/capacityMarketplace.js', 'backend/domain/modelProfiles.js', 'backend/domain/valueCase.js', 'docs/PLAN_BACKLOG_STATUS.md', 'docs/ROADMAP.md', 'docs/REGULATORY_EVIDENCE.md', 'docs/LOCAL_CAPABILITY_MATRIX.md'],
};
...
const valueCase = readFileSync(file('backend/domain/valueCase.js'), 'utf8');
...
check('phase-7:value-case', 'phase-7', valueCase.includes('willingnessToPayValidated') && server.includes('/api/pilots/value-case'), 'pilot economics remain evidence-gated and do not claim willingness to pay');
