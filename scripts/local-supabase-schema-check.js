...
const tables = ['organizations', 'profiles', 'sources', 'alerts', 'cases', 'scenarios', 'audit_log', 'graph_nodes', 'graph_edges', 'action_playbooks', 'action_plans', 'provenance_records', 'data_catalog', 'platform_snapshots', 'action_plan_events', 'decision_shares', 'source_intake_reviews', 'calibration_fixtures', 'incidents', 'notifications', 'webhooks', 'webhook_deliveries', 'job_runs'];
...
const writePolicies = ['analysts can insert action plan events', 'analysts can insert decision shares', 'analysts can update decision shares', 'analysts can insert source intake reviews', 'analysts can update source intake reviews', 'analysts can insert calibration fixtures', 'analysts can update calibration fixtures', 'analysts can insert incidents', 'analysts can update incidents', 'members can update notifications', 'admins can insert webhooks', 'admins can update webhooks', 'analysts can insert webhook deliveries', 'analysts can update webhook deliveries', 'analysts can insert job runs', 'analysts can update job runs'];
...
const roleScopedPolicies = writePolicies.filter((name) => name !== 'members can update notifications');
check('operational-write-policy-scope', roleScopedPolicies.every((name) => {
  const body = policyBody(name);
  return body.includes('organization_id = public.current_organization_id()') && (body.includes("public.current_app_role() in ('admin', 'risk_analyst')") || body.includes("public.current_app_role() = 'admin'"));
}) && policyBody('members can update notifications').includes('organization_id = public.current_organization_id()'), 'every normalized write policy checks tenant and allowed app role');
const failed = checks.filter((item) => item.status === 'fail');
console.log(JSON.stringify({ schemaVersion: '1.3.0-local', checkedAt: new Date().toISOString(), gate: failed.length ? 'FAIL' : 'PASS', files, checks, disclaimer: 'Textual local audit; run it again in Supabase staging before production.' }, null, 2));
