...
function PilotReadinessPanel() {
  ...
  return <div className="bg-panel border border-line rounded-lg p-4"><div className="flex items-start justify-between gap-3"><div><div className="font-mono text-[10px] uppercase tracking-widest text-signal">Pilot readiness</div><h2 className="font-display text-lg font-semibold text-ink mt-1">Preparación verificable del piloto</h2></div><span className={`font-mono text-[10px] ${readiness.customerReady ? 'text-signal' : 'text-alert'}`}>{readiness.customerReady ? 'CUSTOMER READY' : 'EVIDENCIA PENDIENTE'}</span></div><p className="text-xs text-ink-muted mt-2">Captura entrevistas, urgencia, acceso a datos y evidencia económica sin convertir hipótesis en claims comerciales.</p><div className="grid grid-cols-2 md:grid-cols-6 gap-2 mt-3"><Metric label="Gates pass" value={`${passed}/${readiness.checks?.length || 0}`} /><Metric label="Entrevistas" value={readiness.evidenceCounts?.interviews || 0} /><Metric label="Urgentes" value={readiness.evidenceCounts?.urgentInterviews || 0} /><Metric label="Acceso datos" value={readiness.evidenceCounts?.dataAccessEvidence || 0} /><Metric label="Casos" value={metrics.metrics?.casesObserved || 0} /><Metric label="Feedback" value={feedback.length} /></div><div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2">{(readiness.checks || []).map((check) => <div key={check.id} className="border border-line rounded p-2 flex items-center justify-between gap-2 text-xs"><span className="text-ink-muted">{check.label}</span><span className={`font-mono text-[10px] ${check.pass ? 'text-signal' : 'text-alert'}`}>{check.pass ? 'PASS' : 'PENDIENTE'}</span></div>)}</div><form onSubmit={submit} className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-2"><select className="control" value={draft.stage} onChange={(event) => update('stage', event.target.value)}><option value="interview">Entrevista</option><option value="pilot_review">Revisión piloto</option><option value="gate_review">Revisión de gate</option></select><select className="control" value={draft.evidenceType} onChange={(event) => update('evidenceType', event.target.value)}><option value="general">Evidencia general</option><option value="data_access">Acceso a datos</option><option value="economic_value">Valor económico</option><option value="success_criteria">Criterio de éxito</option><option value="adoption">Adopción</option></select><select className="control" value={draft.urgencyScore} onChange={(event) => update('urgencyScore', event.target.value)} aria-label="Urgencia del problema"><option value="1">Urgencia 1/5</option><option value="2">Urgencia 2/5</option><option value="3">Urgencia 3/5</option><option value="4">Urgencia 4/5</option><option value="5">Urgencia 5/5</option></select><input className="control" required value={draft.role} onChange={(event) => update('role', event.target.value)} placeholder="Rol / sponsor" /><input className="control md:col-span-2" required value={draft.summary} onChange={(event) => update('summary', event.target.value)} placeholder="Hallazgo y problema" /><input className="control" required value={draft.evidence} onChange={(event) => update('evidence', event.target.value)} placeholder="Evidencia verificable" /><button className="border border-signal/40 text-signal rounded px-3 py-2 text-xs">Registrar feedback</button></form>{message && <div role="status" className="text-xs text-signal mt-2">{message}</div>}</div>;
}
