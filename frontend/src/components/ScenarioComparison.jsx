...
export default function ScenarioComparison() {
  ...
  return <section className="bg-panel border border-line rounded-lg overflow-hidden"><div className="p-4 border-b border-line flex items-center gap-3"><GitCompareArrows size={16} className="text-signal" /><div><h2 className="font-display font-semibold text-ink">Comparar escenarios</h2><p className="text-xs text-ink-muted mt-1">Evalua perdida, mitigacion y valor protegido lado a lado.</p></div></div><div className="p-4"><div className="flex flex-wrap gap-2">{scenarios.map((scenario) => <button key={scenario.id} onClick={() => toggle(scenario.id)} className={`border rounded px-3 py-2 text-xs ${selected.includes(scenario.id) ? 'border-signal text-signal bg-signal/10' : 'border-line text-ink-muted'}`}>{scenario.name}</button>)}{!scenarios.length && <span className="text-xs text-ink-muted">Guarda un escenario para compararlo.</span>}</div><button onClick={compare} disabled={selected.length < 2 || busy} className="mt-4 bg-signal text-void rounded px-3 py-2 text-xs font-semibold disabled:opacity-50">{busy ? 'Comparando...' : 'Comparar seleccionados'}</button>{error && <div role="alert" className="text-xs text-alert mt-3">No se pudo comparar: {error}</div>}{comparison && <div className="overflow-x-auto mt-4"><table className="w-full text-xs"><thead><tr className="border-b border-line text-left text-ink-dim"><th className="py-2">Escenario</th><th className="py-2">Perdida vs base</th><th className="py-2">Costo vs base</th><th className="py-2">Proteccion vs base</th></tr></thead><tbody>{comparison.deltas.map((delta) => <tr key={delta.id} className="border-b border-line/60"><td className="py-2 text-ink">{comparison.scenarios.find((item) => item.id === delta.id)?.name}</td><td className="py-2 text-alert">{formatUsd(delta.lossVsBaselineUsd)}</td><td className="py-2 text-ink-muted">{formatUsd(delta.mitigationCostVsBaselineUsd)}</td><td className="py-2 text-signal">{formatUsd(delta.protectedValueVsBaselineUsd)}</td></tr>)}</tbody></table></div>}</div></section>;
}
