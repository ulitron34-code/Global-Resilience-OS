...
function EffectiveIndexCards({ items = [] }) {
  return <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mt-4" aria-label="Índice de resiliencia efectiva">
    {items.map((item) => <div key={item.horizonHours} className="border border-signal/30 bg-signal/5 rounded p-3">
      ...
      <div className="text-xs text-ink mt-2">Exposición recuperable estimada</div>
      ...
      <div className="text-[10px] text-ink-dim mt-2">Contrafactual: {item.optionId} · evidencia asumida</div>
    </div>)}
  </div>;
}
