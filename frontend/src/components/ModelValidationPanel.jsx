...
export default function ModelValidationPanel() {
  ...
  const submitFixtures = async (fixtures) => {
    ...
    try {
      ...
      setDraft(emptyDraft);
      setFileName('');
      setMessage(`${fixtures.length} fixture(s) recorded; the sample still requires sufficient provenance and size.`);
      await refresh();
  ...
  const loadFile = async (event) => {
    ...
    try {
      ...
      if (fixtures.length > 50) throw new Error('Calibration import is limited to 50 fixtures per confirmation');
      if (!window.confirm(`Register ${fixtures.length} historical fixture(s)? Only authorized events should be imported.`)) return;
  ...
  return <section className="bg-panel border border-line rounded-lg p-4">
    <div className="flex items-start justify-between gap-3"><div><div className="font-mono text-[10px] uppercase tracking-widest text-signal">Model assurance</div><h2 className="font-display text-lg font-semibold text-ink mt-1">Validation antes de calibrar</h2></div><span className={validation.ready ? 'font-mono text-[10px] text-signal' : 'font-mono text-[10px] text-alert'}>{validation.ready ? 'INVARIANTS PASS' : 'REVIEW'}</span></div>
    <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mt-3"><Metric label="Local tests" value={`${passed}/${validation.tests.length}`} /><Metric label="Fixtures" value={calibration.fixtureCount} /><Metric label="MAE" value={calibration.metrics?.maeUsd == null ? '—' : `$${Math.round(calibration.metrics.maeUsd).toLocaleString()}`} /><Metric label="Mean Error (MAPE)" value={calibration.metrics?.mape == null ? '—' : `${(calibration.metrics.mape * 100).toFixed(1)}%`} /><Metric label="Calibration" value={calibration.status} /></div>
    <p className="text-[11px] text-ink-dim mt-3">Load only authorized historical events. Incomplete fixtures are retained, but excluded from metrics and do not open the production gate.</p>
    ...
    <form onSubmit={submit} className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-2"><input className="control" required value={draft.id} onChange={(event) => setDraft({ ...draft, id: event.target.value })} placeholder="Event ID" aria-label="Historical event ID" /><input className="control" required type="date" value={draft.eventDate} onChange={(event) => setDraft({ ...draft, eventDate: event.target.value })} aria-label="Historical event date" /><input className="control" required type="number" min="0" value={draft.observedImpactUsd} onChange={(event) => setDraft({ ...draft, observedImpactUsd: event.target.value })} placeholder="Observed impact USD" aria-label="Observed impact USD" /><input className="control" required type="number" min="0" value={draft.predictedImpactUsd} onChange={(event) => setDraft({ ...draft, predictedImpactUsd: event.target.value })} placeholder="Predicted impact USD" aria-label="Predicted impact USD" /><input className="control" required value={draft.sourceId} onChange={(event) => setDraft({ ...draft, sourceId: event.target.value })} placeholder="Authorized source ID" aria-label="Authorized source ID" /><input className="control" required value={draft.provenance} onChange={(event) => setDraft({ ...draft, provenance: event.target.value })} placeholder="Evidence reference" aria-label="Evidence reference" /><input className="control" required value={draft.assetId} onChange={(event) => setDraft({ ...draft, assetId: event.target.value })} placeholder="Affected asset" aria-label="Affected asset" /><input className="control" required type="number" min="0" step="0.1" value={draft.durationHours} onChange={(event) => setDraft({ ...draft, durationHours: event.target.value })} placeholder="Duration (h)" aria-label="Event duration in hours" /><input className="control" required value={draft.alternateRoutes} onChange={(event) => setDraft({ ...draft, alternateRoutes: event.target.value })} placeholder="Alternate routes (comma-separated)" aria-label="Alternate routes" /><input className="control md:col-span-2" required value={draft.recoveryOutcome} onChange={(event) => setDraft({ ...draft, recoveryOutcome: event.target.value })} placeholder="Recovery outcome" aria-label="Recovery outcome" /><button className="border border-signal/40 text-signal rounded px-3 py-2 text-xs md:col-span-3">Register historical fixture</button></form>
