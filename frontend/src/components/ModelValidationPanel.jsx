...
export default function ModelValidationPanel() {
  ...
  return <section className="bg-panel border border-line rounded-lg p-4">
    ...
    <div className="grid grid-cols-2 md:grid-cols-5 gap-2 mt-3"><Metric label="Tests locales" value={`${passed}/${validation.tests.length}`} /><Metric label="Fixtures" value={calibration.fixtureCount} /><Metric label="MAE" value={calibration.metrics?.maeUsd == null ? '—' : `$${Math.round(calibration.metrics.maeUsd).toLocaleString()}`} /><Metric label="Error Promedio (MAPE)" value={calibration.metrics?.mape == null ? '—' : `${(calibration.metrics.mape * 100).toFixed(1)}%`} /><Metric label="Calibración" value={calibration.status} /></div>
