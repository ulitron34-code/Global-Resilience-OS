...
export function getDataCatalogReadiness(catalog = CATALOG) {
  const checks = catalog.map((item) => ({ id: item.id, label: item.name, status: item.licenseStatus === 'active' && hasCompleteLicenseMetadata(item) && !isIllustrativeSource(item) ? 'pass' : 'pending', licenseStatus: item.licenseStatus, metadataComplete: hasCompleteLicenseMetadata(item), illustrative: isIllustrativeSource(item), missingLicenseFields: getMissingLicenseFields(item), requiredFor: item.requiredFor }));
