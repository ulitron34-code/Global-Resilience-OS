...
export function buildBacktestReport(fixtures = [], options = {}) {
  ...
  return {
    schemaVersion: '1.0.0-local', scope: options.modelId || 'all',
    status: !enoughSample ? 'insufficient_sample' : improvesBaseline ? 'improves_baseline' : 'does_not_improve_baseline',
    decision: !enoughSample ? 'abstain_for_production' : improvesBaseline ? 'candidate_for_human_review' : 'exploratory_only',
    sample: { input: fixtures.length, eligible: eligible.length, minimum, enough: enoughSample, excludedIllustrativeFixtureCount },
    baseline: { method: 'median_observed_impact', valueUsd: baselineValue, maeUsd: baselineMaeUsd },
    model: { maeUsd: modelMaeUsd, errorsUsd: modelErrors }, improvementPct,
    fixtures: eligible.map((item) => ({ id: item.id, eventDate: item.eventDate, observedImpactUsd: Number(item.observedImpactUsd), predictedImpactUsd: Number(item.predictedImpactUsd), errorUsd: Math.abs(Number(item.predictedImpactUsd) - Number(item.observedImpactUsd)), sourceId: item.sourceId || null })),
    disclaimer: 'Backtest local; requiere eventos historicos representativos, procedencia verificable y revision experta. No demuestra precision comercial.'
  };
}
