...
app.get('/api/cases/:id/decision-package', authIfConfigured, roleIfConfigured('admin', 'risk_analyst', 'viewer'), (req, res) => {
  ...
  const capacityInquiries = listCapacityInquiries(organizationId).filter((inquiry) => inquiry.caseId === req.params.id);
  const enriched = { ...item, actionPlans, capacityInquiries, actionPlanEvidenceSummary: summarizeActionPlanEvidence(actionPlans), recoveryProfile: buildRecoveryProfile({ cableId: 'seamewe3', severity: 'total', horizons: [24, 168, 720] }), regulatoryEvidenceMap: buildRegulatoryEvidenceMap({ scope: req.params.id, evidence: [] }), packageCapabilities: ['case', 'alert', 'sources', 'models', 'scenarios', 'audit', 'comments', 'action_plans', 'capacity_inquiries', 'recovery_counterfactual', 'regulatory_evidence'] };
...
app.get('/api/ops/source-health', authIfConfigured, roleIfConfigured('admin', 'risk_analyst', 'viewer'), (req, res) => res.json(getSourceHealthOverview(Date.now(), req.user?.organizationId || DEFAULT_ORGANIZATION_ID)));
...
app.get('/api/sources', authIfConfigured, roleIfConfigured('admin', 'risk_analyst', 'viewer'), (req, res) => res.json(listSources(req.user?.organizationId || DEFAULT_ORGANIZATION_ID)));
app.get('/api/sources/:id', (req, res) => {
  const source = getSource(req.params.id, req.user?.organizationId || DEFAULT_ORGANIZATION_ID);
