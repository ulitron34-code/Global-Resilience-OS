...
app.get('/api/data-catalog/intake-reviews', authIfConfigured, roleIfConfigured('admin', 'risk_analyst', 'viewer'), (req, res) => res.json(listSourceIntakeReviews(req.query)));
app.post('/api/data-catalog/intake-reviews', authIfConfigured, roleIfConfigured('admin', 'risk_analyst'), (req, res) => { try { res.status(201).json(createSourceIntakeReview(req.body || {}, req.user?.email || 'operator')); } catch (error) { res.status(400).json({ error: error.message }); } });
app.patch('/api/data-catalog/intake-reviews/:id', authIfConfigured, roleIfConfigured('admin', 'risk_analyst'), (req, res) => { try { const item = updateSourceIntakeReview(req.params.id, req.body || {}, req.user?.email || 'operator'); if (!item) return res.status(404).json({ error: 'Revisión de fuente no encontrada' }); res.json(item); } catch (error) { res.status(400).json({ error: error.message }); } });
...
app.get('/api/incidents', authIfConfigured, roleIfConfigured('admin', 'risk_analyst', 'viewer'), (req, res) => res.json(listIncidents(req.query)));
app.post('/api/incidents', authIfConfigured, roleIfConfigured('admin', 'risk_analyst'), (req, res) => { try { res.status(201).json(createIncident(req.body || {}, req.user?.email || 'operator')); } catch (error) { res.status(400).json({ error: error.message }); } });
app.patch('/api/incidents/:id', authIfConfigured, roleIfConfigured('admin', 'risk_analyst'), (req, res) => { try { const item = updateIncident(req.params.id, req.body || {}, req.user?.email || 'operator'); if (!item) return res.status(404).json({ error: 'Incidente no encontrado' }); res.json(item); } catch (error) { res.status(400).json({ error: error.message }); } });
